...
def update_metadata_instruction_data(
    name, symbol, uri, creators, verified, share
):
    _data = (
        bytes([1])
        + _get_data_buffer(name, symbol, uri, creators, verified, share)
        + bytes([0, 0])
    )
    instruction_layout = cStruct(
        "instruction_type" / Int8ul,
...
def create_master_edition_instruction(
    ...
    keys = [
        ...
        AccountMeta(
            pubkey=PublicKey(SYSTEM_PROGRAM_ID),
            is_signer=False,
            is_writable=False,
        ),
        AccountMeta(
            pubkey=PublicKey(SYSVAR_RENT_PUBKEY),
            is_signer=False,
            is_writable=False,
        ),
    ]
